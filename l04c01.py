...
class Graph:
    def __init__(self, adjacency_matrix=[]):
        ...
        for i in range(len(self.adjacency_matrix)):
            for j in range(len(self.adjacency_matrix)):
                if self.adjacency_matrix[i][j] not in [None, 0]:
        ...
        while len(queue) != 0:
            ...
            for neighbour in vert_neighbours:
                if (
                    ...
                    and abs(self.adjacency_matrix[current_vert][neighbour]) > 0
                ):
    ...
    def dfs(self, vert1, vert2=None, delta=0, adjMatrix=None):
        ...
        def dfs_recursive(vert):
            ...
            for neighbour in unvisited_vert_neighbours:
                if abs(adjMatrix[vert][neighbour]) >= min_flow_val:
                    prev[neighbour] = vert
    ...
    def augment_path(self, path, adjMatrix=None):
        ...
        for edge in edges:
            # If the edge from the WORKING graph is normal we subtract from normal and residual
            # Else we add to residual and normal
            # If project_changes, we augment the flow in the passed matrix also (sign accounted for in the if)
            if adjMatrix[edge[0]][edge[1]] > 0:
                self.adjacency_matrix[edge[0]][edge[1]] -= max_path_flow
                self.adjacency_matrix[edge[1]][edge[0]] -= max_path_flow
                if project_changes:
...
def solution(sources, sinks, adjMatrix):
    ...
    graph = Graph(adjacency_matrix=adjMatrix)
